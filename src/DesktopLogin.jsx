  ...
  return (
    <div className="desktop-login">
      <header className="desktop-login__header">
        ...
        />
        <div className="desktop-login__language-switcher" aria-label={t.language}>
          {LOGIN_LANGUAGE_OPTIONS.map((option, index) => (
            <div key={option.value} className="desktop-login__language-item">
              <button
                type="button"
                className={`desktop-login__language-button ${language === option.value ? 'is-active' : ''}`}
              ...
              {index < LOGIN_LANGUAGE_OPTIONS.length - 1 && (
                <span className="desktop-login__language-separator" aria-hidden="true">/</span>
              )}
            </div>
          ))}
        </div>
