...
import { ChevronLeft, ArrowUp, Maximize2, Minimize2, PenLine, Trash2, X } from 'lucide-react';
...
const normalizeTodoRecord = (todo) => {
  ...
  return {
    ...
    updatedAt: typeof todo.updatedAt === 'string' && todo.updatedAt.trim() ? todo.updatedAt : null,
...
const getBlockTodosInDisplayOrder = (todos, dateString, timeOfDay) => todos
  ...
  .map(({ todo }) => todo);
const getDayTodosInDisplayOrder = (todos, dateString) => todos
  .map((todo, index) => ({ todo, index }))
  .filter(({ todo }) => todo.dateString === dateString)
  .sort((a, b) => {
    const timeRankA = DAY_TASK_TIME_ORDER.indexOf(a.todo.timeOfDay);
    const timeRankB = DAY_TASK_TIME_ORDER.indexOf(b.todo.timeOfDay);
    ...
    const slotA = isValidDesktopSlot(a.todo.desktopSlot) ? a.todo.desktopSlot : Number.POSITIVE_INFINITY;
    const slotB = isValidDesktopSlot(b.todo.desktopSlot) ? b.todo.desktopSlot : Number.POSITIVE_INFINITY;
...
function MobileApp({ session, platformInfo }) {
  ...
  const scrollToCurrentTime = useCallback((behavior = 'smooth') => {
    ...
    if (timelineRef.current) {
      timelineRef.current.scrollTo({ top: 0, behavior });
    }
  }, [selectedDate]);
  ...
  const handleAddTodo = () => {
    ...
    const resolvedBlock = isSameDay(selectedDate, getLogicalToday()) ? getCurrentTimeBlock() : 'Morning';
    ...
    if (cardType === 'video' && videoUrl) {
      fetchVideoMeta(videoUrl).then(meta => {
        setTodos(prev => prev.map(t => t.id === newTodoId ? normalizeTodoRecord({ ...t, ...meta, updatedAt: operationUpdatedAt }) : t));
      });
    ...
    } else if (typeFields.primaryUrl && (!cardType || cardType === 'link' || cardType === 'text' || cardType === 'ai_tool' || cardType === 'social' || cardType === 'shopping' || cardType === 'financial' || cardType === 'document')) {
  ...
  const finalizeTouchDrag = useCallback((todoId) => {
    ...
    if (el) {
      resetDragSourceCardState(todoId);
    }
  ...
  useLayoutEffect(() => {
    if (!dragOverlayTodo) return undefined;
  ...
  const handleEditSave = () => {
    ...
    setTodos(prev => prev.map(t =>
      t.id === editingTodo.id ? normalizeTodoRecord({ ...t, text: rawText, updatedAt: operationUpdatedAt, ...typeFields }) : t
    ));
    if (typeFields.cardType === 'video' && typeFields.videoUrl) {
      fetchVideoMeta(typeFields.videoUrl).then(meta => {
        setTodos(prev => prev.map(t => t.id === editingTodo.id ? normalizeTodoRecord({ ...t, ...meta, updatedAt: operationUpdatedAt }) : t));
      });
    ...
    } else if (typeFields.primaryUrl && (!typeFields.cardType || typeFields.cardType === 'link' || typeFields.cardType === 'text' || typeFields.cardType === 'ai_tool' || typeFields.cardType === 'social' || typeFields.cardType === 'shopping' || typeFields.cardType === 'financial' || typeFields.cardType === 'document')) {
      fetchLinkPreviewMeta(typeFields.primaryUrl).then(meta => {
        if (meta) {
          setTodos(prev => prev.map(t => t.id === editingTodo.id ? normalizeTodoRecord({ ...t, ...meta, updatedAt: operationUpdatedAt }) : t));
        }
        ...
        >
          {faviconUrl && !faviconError ? (
            <img
              ...
              style={{ width: 18, height: 18, borderRadius: 3, objectFit: 'contain', position: 'relative', zIndex: 10 }}
  ...
  const renderTimelineBlocks = useCallback((date, options = {}) => {
    ...
    const orderedTodos = getDayTodosInDisplayOrder(todos, dateKey);

    return (
      <div
        className="tasks-col"
        key={`list-${dateKey}`}
        data-drag-target-disabled="true"
        style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: '4px 20px 28px' }}
      >
          ...
          return (
            <div key={todo.id} id={`swipe-wrapper-${todo.id}`} data-swipe-wrapper-id={todo.id} className="swipe-wrapper">
              ...
              <div
                ...
                className={`task-card ${isPast ? 'is-past' : ''}`}
                onClick={() => {
  ...
  }, [closeSwipeActions, currentTime, deleteTodo, getSwipeHandlers, handleTaskEditAction, handleTaskPrimaryAction, renderTaskCardInner, session?.user?.id, todos, language]);
    ...
    <>
                        ...
                        {isLanguageDropdownOpen && (
                          <div className="language-dropdown-menu">
                            {PROFILE_LANGUAGE_OPTIONS.map((option) => (
                              <div key={option.value} className={`language-option ${language === option.value ? 'selected' : ''}`} onClick={() => { setLanguage(option.value); setIsLanguageDropdownOpen(false); }}>
                                <span>{option.label}</span>
                                {language === option.value && <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>}
                              </div>
                            ))}
                          </div>
                        )}
      ...
      <MobileHistoryModal
        ...
        onTaskClick={(todo) => {
          ...
          if (redirectUrl) {
            if (normalizeCardType(todo.cardType) === CARD_TYPES.PHOTO) {
              setFullscreenImage(todo.photoUrl || todo.photoDataUrl || redirectUrl);
              return;
            }
